...
function codeGen(script, pretty) {
  var ts = new _token_stream.TokenStream();
  var rep = (0, _shiftReducer["default"])(pretty ? PRETTY : COMPACT, script);
  rep.emit(ts);
  return ts.result;
}
...
var CodeGen = (function () {
  ...
  _createClass(CodeGen, [{
    ...
    value: function reduceObjectBinding(node, _ref8) {
      ...
      var state = this.brace(commaSep(properties));
    ...
    value: function reduceBlock(node, _ref11) {
      ...
      return this.brace(seq.apply(undefined, _toConsumableArray(statements)));
    }
  }, {
    ...
    value: function reduceFunctionExpression(node, _ref31) {
      ...
      var state = seq(t("function"), node.isGenerator ? t("*") : empty(), name ? name : empty(), paren(params), this.brace(body));
      state.startsWithFunctionOrClass = true;
      return state;
    }
  }, {
    ...
    value: function reduceImportNamespace(node, _ref37) {
      ...
      return seq(t("import"), defaultBinding == null ? empty() : seq(defaultBinding, t(",")), t("*"), t("as"), namespaceBinding, t("from"), t(escapeStringLiteral(node.moduleSpecifier)), this.semiOp());
    }
  }, {
    ...
    value: function reduceSwitchStatement(node, _ref55) {
      ...
      return seq(t("switch"), paren(discriminant), this.brace(seq.apply(undefined, _toConsumableArray(cases))));
    }
  }, {
    ...
    value: function reduceSwitchStatementWithDefault(node, _ref56) {
      ...
      return seq(t("switch"), paren(discriminant), this.brace(seq.apply(undefined, _toConsumableArray(preDefaultCases).concat([defaultCase], _toConsumableArray(postDefaultCases)))));
    }
  }, {
...
var FormattedBrace = (function (_CodeRep13) {
  _inherits(FormattedBrace, _CodeRep13);

  function FormattedBrace(expr) {
    _classCallCheck(this, FormattedBrace);

    _get(Object.getPrototypeOf(FormattedBrace.prototype), "constructor", this).call(this);
    this.expr = expr;
  }

  _createClass(FormattedBrace, [{
    key: "emit",
    value: function emit(ts) {
      ts.put("{\n");
      this.expr.emit(ts, false);
  ...
  return FormattedBrace;
})(CodeRep);

var FormattedSemiOp = (function (_CodeRep14) {
  _inherits(FormattedSemiOp, _CodeRep14);

  function FormattedSemiOp() {
    _classCallCheck(this, FormattedSemiOp);

    _get(Object.getPrototypeOf(FormattedSemiOp.prototype), "constructor", this).call(this);
  }

  _createClass(FormattedSemiOp, [{
    key: "emit",
    value: function emit(ts) {
      ts.putOptionalSemi();
      ts.put("\n");
  ...
  return FormattedSemiOp;
})(CodeRep);

var FormattedCodeGen = (function (_CodeGen) {
  _inherits(FormattedCodeGen, _CodeGen);
...
var COMPACT = new CodeGen();
var PRETTY = new FormattedCodeGen();
